...
を置いて（先述した環境構築が完了すると置かれ）ます。  
開発はapp（名称変更可）/内で行います。


### bower.json

インストールするライブラリを管理しています。  
開発環境では"dependencies"内に記述されているライブラリを個別に読み込みます。  
デプロイ時には"dependencies"内のファイルを連結して出力します。minify設定は後述します。

新たにライブラリをインストールする場合は
...
ruby-sassを使用していますので、その設定ファイルです。  
現状「line_comments」の設定しか使用していません。


### gulpfile.coffee

タスクはすべてgulp/tasks内の各種ファイルで設定しています。  
ここではタスクのディレクトリを指定しているだけなので、基本編集は必要ありません。  
gulpの設定はすべてCoffeeScriptで記述しています。
...
ディレクトリ名やパラメータを定義していて各タスクから参照しています。  
データはすべて後述するdata/init.yamlにて管理しています。
...
data/init.yamlを参照し、各ectファイルをhtmlにして吐き出します。  
詳しくは後述します。


##### watch.coffee

監視するファイルと、監視ファイルに変更が加わった際に起動するタスクを指定しています。  
imagesは"config.path"を使うと上手く動いてくれなかったため"config.dir"で指定しています。
